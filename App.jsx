...
const SHARED_STYLES = `
  ...
  * { box-sizing:border-box; margin:0; padding:0; }
  ...
  .field label { font-size:12.5px; font-weight:600; color:var(--ink); letter-spacing:.02em; }
  ...
  .auth-error { background:#fdf0ed; border:1px solid #e8a07f; border-radius:8px; padding:9px 12px;
    font-size:13px; color:#b3261e; margin-bottom:12px; }
  .auth-success { background:#edf7f0; border:1px solid #7fc49a; border-radius:8px; padding:12px 14px;
    font-size:13.5px; color:#2d6a45; margin-bottom:12px; line-height:1.5; }
  ...
  /* ── App header ── */
  .head { margin-bottom:18px; border-bottom:1px solid var(--line); padding-bottom:16px; display:flex; align-items:flex-start; justify-content:space-between; gap:12px; flex-wrap:wrap; }
  .head-left {}
  ...
  /* ── Insight ── */
  .insight { background:linear-gradient(180deg,#fff6ef,#fffdf8); border:1px solid #e8a07f; border-radius:12px; padding:14px 16px; margin-bottom:16px; font-size:14.5px; line-height:1.5; }
...
// ── Opcoes extraído fora de App para evitar remount ──
...
function TelaLogin({ onLogin, onForgot }) {
  ...
  const handleSubmit = (e) => {
    ...
    setTimeout(() => { setCarregando(false); onLogin(email); }, 800);
  };
  ...
  return (
    <div className="auth-wrap">
      ...
      <div className="auth-box">
        ...
        <p className="auth-sub">Entre com sua conta para acessar a ferramenta de priorização.</p>
...
export default function App() {
  const [tela, setTela] = useState("login"); // "login" | "esqueceu" | "app"
  ...
  if (tela === "login") {
    return (
      <TelaLogin
        onLogin={(email) => { setUsuario(email); setTela("app"); }}
        onForgot={() => setTela("esqueceu")}
      />
    );
  }

  if (tela === "esqueceu") {
    return <TelaEsqueceuSenha onVoltar={() => setTela("login")} />;
  }
    ...
    <div className="wrap">
      ...
      <div className="head">
        <div className="head-left">
          ...
          <h1>Matriz GUT</h1>
          <p className="sub">
            Para cada problema, toque na alternativa de <b>Gravidade</b>,{" "}
            <b>Urgência</b> e <b>Tendência</b>. A prioridade é{" "}
            <b>GUT = G × U × T</b> — quanto maior, mais cedo deve ser atacado.
          </p>
        </div>
